...
// WriteVarsFile writes playbooks/vars/zfs_allowed_values.yml from the current
// property definitions. Playbooks include this file to validate property values
// before passing them to the ZFS CLI. The file is regenerated on every startup
// so it always matches the schema in the binary.
func WriteVarsFile(playbooksDir string) error {
	varsDir := filepath.Join(playbooksDir, "vars")
	if err := os.MkdirAll(varsDir, 0o755); err != nil {
		return fmt.Errorf("create vars dir: %w", err)
	}

	var b strings.Builder
	b.WriteString("# Auto-generated by dumpstore on startup — do not edit manually.\n")
	for _, p := range allProperties {
		if p.InputType != "select" || len(p.Options) == 0 {
			continue
		}
		vals := make([]string, len(p.Options))
		for i, o := range p.Options {
			vals[i] = fmt.Sprintf("%q", o.Value)
		}
		fmt.Fprintf(&b, "zfs_allowed_%s: [%s]\n", p.Name, strings.Join(vals, ", "))
	}

	dest := filepath.Join(varsDir, "zfs_allowed_values.yml")
